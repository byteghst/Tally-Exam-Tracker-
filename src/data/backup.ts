...
export function downloadBackupFile(backup: Backup, filename = `exam-tracker-backup-${Date.now()}.json`) {
...
export function validateBackup(raw: unknown): { ok: true; data: Backup } | { ok: false; error: string } {
  ...
  if (!parsed.success) {
    return { ok: false, error: 'This file is not a valid Exam Tracker backup.' };
  }
